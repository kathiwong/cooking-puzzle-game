...
public class TileBoard : MonoBehaviour
{
    ...
    public void Update() //run every frame
    {
        if (!waiting)   //respond to user input only if not waiting for tiles to finish moving
        {
            //check empty board
            if (IsEmpty())
            {
            ...
            //arrow keys or WASD input for moving tiles
            if (Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.UpArrow))   //check if the space key is pressed
            {
    ...
    private void MoveTiles(Vector2Int direction, int startX, int incrementX, int startY, int incrementY)   // Logic to move tiles in the specified direction
    {
        bool changed = false;
    ...
    {
        //return a.number == b.number && !b.locked;   //can merge if both tiles have the same number

        if (b.number >= MaxTileValue) //if the tile is already at the maximum value, it cannot be merged further
        {
            return false;
        }
        return a.number == b.number && !b.locked;   //can merge if both tiles have the same number and the target tile is not locked
    }
